        ...
        private static int GetDataAndSetGroups(List<Group> allGroups)
        {
            ...
            while (true)
            {
                ...
                if (command.Contains("=>"))
                {
                    ...
                    towns.Add(new Town() { Name = townAndSeats[0],
                                           SeatsAvalable = int.Parse(townAndSeats[1].Substring(0, townAndSeats[1].IndexOf(' '))) });
                }
                else
                {
                    ...
                    towns[towns.Count - 1].Students.Add(new Student() { Name = nameEmailDate[0],
                        Email = nameEmailDate[1],
                        DateOfRegistration = DateTime.ParseExact(nameEmailDate[2], "d-MMM-yyyy", CultureInfo.InvariantCulture) });
                }
            }
