namespace _05._Book_Library
{
    ...
    public class Program
    {
        ...
        private static void PrintPricesByAuthor(Library gatheredBooks)
        {
            ...
            {
                if (moneyByAuthors.ContainsKey(book.Author))
                {
                    moneyByAuthors[book.Author] += book.Price;
                }
                else
                {
                    moneyByAuthors[book.Author] = book.Price;
        ...
        private static Library FillLibrary()
        {
            ...
            for (int i = 0; i < numberOfBooks; i++)
            {
                ...
                Book temp = new Book(bookData[0], bookData[1], bookData[2], 
                    DateTime.ParseExact(bookData[3], "d.M.yyyy", CultureInfo.InvariantCulture), bookData[4], double.Parse(bookData[5]));
