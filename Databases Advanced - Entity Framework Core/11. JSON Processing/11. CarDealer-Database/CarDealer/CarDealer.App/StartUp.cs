namespace CarDealer.App
{
    ...
    public class StartUp
    {
        ...
        private static void ExportData(CarDealerContext context, string baseDir = @"..\..\..\JsonResults\")
        {
            var orderedCustomersString = Serializer.OrderedCustomers(context);
            File.WriteAllText(baseDir + "ordered-customers.json", orderedCustomersString);

            var carsFromMakeToyotaString = Serializer.CarsFromMakeToyota(context);
        ...
        {
            var suppliers = Deserializer.ImportSuppliers(File.ReadAllText(baseDir + "suppliers.json"));

            var parts = Deserializer.ImportParts(suppliers.ToArray(), File.ReadAllText(baseDir + "parts.json"));

            var cars = Deserializer.ImportCars(parts, File.ReadAllText(baseDir + "cars.json"));

            var customers = Deserializer.ImportCustomer(File.ReadAllText(baseDir + "customers.json"));
